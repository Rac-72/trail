...
var textObj = { name: "quincy", legs: 4, tails: 1 };
...
var myMusic = [
  ...
  },
  {
    ...
    title: "ligaligali",
    "release-year": 2003,
    formats: ["youtube", "instagram", "boomplay"],
  },
];
...
const realNumbers = [4, 2.6, 7.8, 16, 4, -2, 0.45, 5];
const squareList = (arr) => {
  const squareInterger = arr
...
nur.sort(function (a, b) {
  return a - b;
});
console.log(nur);
let sur = [1, 200, 40, 50, 60, 70, 100, 500];
sur.sort(function (a, b) {
  return b - a;
});
console.log(sur);
...
function myNote() {
  ...
  if (inputValue === "") {
    alert("pls Enter Expenses");
  } else {
    document.getElementById("thisnote").appendChild(tr);
    inputValue++;
  }
  ...
  let c = document.createTextNode(dateValue);
  vr.appendChild(c);

  if (dateValue === "") {
    alert("pls Enter Expenses");
  } else {
    document.getElementById("thisnote").appendChild(vr);
    dateValue++;
  }

  document.getElementById("mydate").value = "";

  let wr = document.createElement("th");
  let amountValue = document.getElementById("myamount").value;

  let d = document.createTextNode(amountValue);
  wr.appendChild(d);

  if (amountValue === "") {
    alert("pls Enter Expenses");
  } else {
    document.getElementById("thisnote").appendChild(wr);
    a;
  }
...
function rot13(str) {
  ...
  let convertl = convert.length;
  let strl = str.length;
  let newstr = "";
  for (let i = 0; i < strl; i++) {
    for (let j = 0; j < convertl; j++) {
      if (str[i] == convert[j][0]) {
        newstr += convert[j][1];
      }
    }
  }
...
console.log(roll13("ABOSED AFOLABI"));
...
function checkCashRegister(price, cash, cid) {
  ...
  filteredCid.forEach((elem) => {
    ...
    cidSum += currValue;
    let amount = 0;
    while (changeToGive >= currencyUnit[currency] && currValue > 0) {
      amount += currencyUnit[currency];
      changeToGive -= currencyUnit[currency];
      currValue -= currencyUnit[currency];
    }
    if (amount !== 0) {
  ...
  if (changeToGive > 0) {
    ...
    change = [];
  } else if (changeToGive == 0 && changeCheck == cidSum) {
    status = "CLOSED";
    change = cid;
  } else {
    status = "OPEN";
...
console.log(checkCashRegister(20, 20, [
    ["PENNY", 1.01],
    ["NICKEL", 2.05],
    ["DIME", 3.1],
    ["QUARTER", 4.25],
 ...
 return  m === null?"has no wowels":`${str}`+"is"+ m.length;
 }
  console.log(vowels(""))